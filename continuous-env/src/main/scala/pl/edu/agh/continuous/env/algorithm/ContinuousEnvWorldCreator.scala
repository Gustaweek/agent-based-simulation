...
object ContinuousEnvWorldCreator extends WorldCreator[ContinuousEnvConfig] {
  ...
  override def prepareWorld()(implicit config: ContinuousEnvConfig): WorldBuilder = {
    ...
    while (cellQueue.nonEmpty) {
      ...
      val continuousEnvCell: ContinuousEnvCell = if (x == 27 && y == 40) {
    ...
    while (finalCellQueue.nonEmpty) {
      ...
      if (gridMultiCellId.x == 24 && gridMultiCellId.y == 28) {
        val runner1: Runner = Runner.createNew(Vec2(20, 20), 17, 1000, Color.ORANGE)
        addRunnersToGuiMap(continuousEnvCell, runner1)
      }
      if (gridMultiCellId.x == 24 && gridMultiCellId.y == 30) {
        val runner1: Runner = Runner.createNew(Vec2(20, 20), 17, 1000, Color.RED)
        addRunnersToGuiMap(continuousEnvCell, runner1)
      }
      if (gridMultiCellId.x == 24 && gridMultiCellId.y == 32) {
        val runner1: Runner = Runner.createNew(Vec2(20, 20), 17, 1000, Color.GREEN)
        addRunnersToGuiMap(continuousEnvCell, runner1)
      }
      if (gridMultiCellId.x == 24 && gridMultiCellId.y == 26) {
        val runner1: Runner = Runner.createNew(Vec2(20, 20), 17, 1000, Color.PINK)
        addRunnersToGuiMap(continuousEnvCell, runner1)
      }


      if (gridMultiCellId.x == 22 && gridMultiCellId.y == 22) {
        val runner1: Runner = Runner.createNew(Vec2(20, 20), 17, 1000, Color.LIGHT_GRAY)
        addRunnersToGuiMap(continuousEnvCell, runner1)
      }
      if (gridMultiCellId.x == 22 && gridMultiCellId.y == 25) {
        val runner1: Runner = Runner.createNew(Vec2(20, 20), 17, 1000, Color.BLUE)
        addRunnersToGuiMap(continuousEnvCell, runner1)
      }
      if (gridMultiCellId.x == 22 && gridMultiCellId.y == 26) {
        val runner1: Runner = Runner.createNew(Vec2(20, 20), 17, 1000, Color.CYAN)
        addRunnersToGuiMap(continuousEnvCell, runner1)
      }
      if (gridMultiCellId.x == 22 && gridMultiCellId.y == 27) {
        val runner1: Runner = Runner.createNew(Vec2(20, 20), 17, 1000, Color.DARK_GRAY)
        addRunnersToGuiMap(continuousEnvCell, runner1)
      }
      if (gridMultiCellId.x == 22 && gridMultiCellId.y == 20) {
        val runner1: Runner = Runner.createNew(Vec2(20, 20), 17, 1000, Color.BLACK)
        addRunnersToGuiMap(continuousEnvCell, runner1)
      }


      if (gridMultiCellId.x == 40 && gridMultiCellId.y == 28) {
